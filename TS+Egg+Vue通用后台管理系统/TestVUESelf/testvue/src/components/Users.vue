       ...
       // 编辑用户相关代码
        ...
        //提交编辑之后的数据
         private editUser(){
            ...
            this.form!.validate((flag)=>{
                if(flag){
                    updateUsers((this.editData as any).id, this.editData)
                        .then((response:any)=>{
                            if(response.status === 200){
                                const idx = this.tableData.findIndex((obj)=>{
                                    return (obj as any).id === (this.editData as any).id;
                                });
        ...
        private beforeExcelUpload(file:any) {
            console.log("exeecl文件类型",file.type);
